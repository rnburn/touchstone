...
class PositiveIntegralOption {
 public:
  PositiveIntegralOption(int value) : _value(value) {
    if (value <= 0) {
...
inline void set_benchmark_option(BenchmarkSet& benchmark_set,
...
template <class T,
          typename std::enable_if<std::is_base_of<EnumerationRange, T>::value,
                                  int>::type = 0>
void set_benchmark_option(BenchmarkSet& benchmark_set, T enumeration_range) {
  benchmark_set.set_enumeration_range(
      std::make_shared<const T>(enumeration_range));
}

template <class Duration>
inline void set_benchmark_option(BenchmarkSet& benchmark_set, Units<Duration>) {
  benchmark_set.set_elapse_recorder(make_elapse_recorder<Duration>());
}

inline void set_benchmark_options(BenchmarkSet& benchmark_set) {}
...
void set_benchmark_options(BenchmarkSet& benchmark_set, Option option,
